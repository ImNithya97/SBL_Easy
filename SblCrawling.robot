...
Open the website and search by year
    ...
    Wait Until Keyword Succeeds    15x   2s   Click Element When Visible     //*[@id="yearofdeath"]
...
#------------------------------create the year list  and get the count-------------------------------------------#
    ...
    FOR    ${row}    IN   @{type_list}
        ...
        Wait Until Keyword Succeeds    2x   2s   Click Element When Visible          //*[@id="yearofdeath"]//option[@value=${row}]
        sleep   1s
        ...
        ${MsgBoxPresent}     Is Element Visible        //*[contains(text(),"Your search returned too many results.")]
        IF    ${MsgBoxPresent}
...
Navigate through search results and write to excel
    ${status}   ${out}     Run Keyword And Ignore Error      Wait Until Element Is Visible    //*[@id="rhs"]/div[3]/div/table/tbody/tr
    IF  '${status}' == 'FAIL'
        Return From Keyword
    ...
    #    ${table}=       Create Table  columns=${header_list} 
        ...
        FOR    ${j}    IN RANGE    0     5     1
            ...
            Scroll Element Into View     //*[@id="rhs"]/div[3]/div/table/tbody/tr[${j+1}]
            #-----------------------------extract the record url-----------------------------------------
            ${Ext_Record_url}        Get Element Attribute    //*[@class="resultswrap"]/table/tbody/tr[${j+1}]/td[1]/a        href
            Wait Until Keyword Succeeds    2x   2s    Click Element When Visible           //*[@id="rhs"]/div[3]/div/table/tbody/tr[${j+1}]
            #---------------------------------extract the Book details-------------------------------------------------#
            Book Details         ${Ext_Record_url}   
            Wait Until Keyword Succeeds    2x   2s   Click Element When Visible   //*[@class="breadcrumbs"]//*[contains(text(),"Search results")]
...
Navigate through pages
    ...
    FOR    ${k}    IN RANGE    1     1     1
        ...
        Navigate through search results and write to excel
    END
    Wait Until Keyword Succeeds    2x   2s   Click Element When Visible     //*[@class="breadcrumbs"]//*[contains(text(),"Roll Search")]
